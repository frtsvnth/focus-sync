...
2. Собираются праздники из трёх источников (в порядке приоритета):
   - **Локальная база** (~90 записей, покрывает все месяцы) — основной источник
   - **Wikimedia API** (`api.wikimedia.org`) — «в этот день», работает из РФ без VPN, CORS *
   - **Причудливый список** (24 неформальных «праздника») — fallback для дней без событий
...
Браузер (index.html)
    ...
    ├── api.wikimedia.org (праздники, CORS *)
    └── Cloudflare Worker (CORS-прокси)
...
| Wikimedia | REST API, CORS * | База праздников «в этот день», прямой доступ из браузера |
