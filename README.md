...
2. Собираются праздники из трёх источников (в порядке приоритета):
   - **Локальная база** (~70 записей, покрывает все месяцы) — основной источник
   - **Nager.Date API** (`date.nager.at`) — международная база гос. праздников, **нестабилен из РФ**
...
Браузер (index.html)
    ...
    ├── date.nager.at (праздники, опционально)
...
| Nager.Date | REST API | База гос. праздников (резервный источник) |
